...
Camera::Camera(GLFWwindow* pWin, float farPlaneDist) : m_Position(0.0f,5.0f,5.0f), m_Target(0.0f,0.0f,0.0f), m_Up(0.0f,1.0f,0.0f), m_LastMouseX(-1), m_LastMouseY(-1), m_Panning(0,0,0), m_Zoom(0,0,0), m_Rotation(0,0,0), WindowWidth(640), WindowHeight(480), pWindow(pWin)
{
    if(pWindow)
    ...
    // Edit von Justin: Far plane Koordinaten weiter weg machen, damit die Clipping planes nicht sofort gerendert werden...Dachte eigentlich das würde mit glFrustum gemacht werden?
    m_ProjMatrix.perspective((float)M_PI*65.0f/180.0f, (float)WindowWidth/(float)WindowHeight, 0.045f, farPlaneDist);
}
...
void Camera::mouseInput( int x, int y, int Button, int State)
{
    ...
    if(State == GLFW_PRESS)
    {
        ...
        {
            pan( (float)(m_LastMouseX-x)*0.01f * 10, (float)(m_LastMouseY-y)*0.01f * 10 );
        }
        else if( Button == GLFW_MOUSE_BUTTON_MIDDLE)
        {
            zoom( (float)(m_LastMouseY-y)*0.01f * 10);
